...
MY_ID=7679796977
# Persistent storage - try multiple paths for justrunmyapp 24h restart survival
STATE_PATHS=["/data/bot_state.json", "/app/data/bot_state.json", "bot_state.json", "/tmp/bot_state.json"]
...
async def get_real_balance():
    if not PASSPHRASE or len(PASSPHRASE)<2:
        return None, "PASSPHRASE missing! Add BITGET_PASSPHRASE in justrunmyapp Env Vars"
    ...
    if data and data.get("code")=="00000":
        ...
        return 0.0, None
    return None, f"{data.get('code')}:{data.get('msg')} (Check API_KEY/SECRET/PASSPHRASE)" if data else "Fail"
...
def calc_signal_hybrid(closes):
    ...
    # SELL: Loosened to hold longer for bigger profit - fixes quick close
    sell_cond=False
    sell_reason=""
    if sma10 and sma30 and prev_sma10 and prev_sma30:
        death_cross = prev_sma10 >= prev_sma30 and sma10 < sma30
        overbought_extreme = rsi > 85  # was 70 - now 85
        overbought_with_cross = rsi > 75 and death_cross
        below_trend_strong = price < sma30 * 0.985  # 1.5% below sma30, not just below
        if death_cross and overbought_extreme:
            sell_cond=True
            sell_reason=f"RSI {rsi:.0f} extreme + death cross"
        elif overbought_with_cross:
            sell_cond=True
            sell_reason=f"RSI {rsi:.0f} overbought + death cross"
        elif overbought_extreme:
            sell_cond=True
            sell_reason=f"RSI {rsi:.0f} extreme overbought"
        elif below_trend_strong and death_cross:
            sell_cond=True
            sell_reason=f"Strong downtrend + death cross"
        elif below_trend_strong and rsi > 65:
            sell_cond=True
            sell_reason=f"Below trend RSI {rsi:.0f}"
...
async def auto_loop(app):
    while True:
        ...
        if len(closes)>=5:
            ...
            if abs(chg5)>=15: # significant move
                ...
                if chg5>0 and sig=="BUY":
                    ...
                    save_state(s)
                    try: await app.bot.send_message(chat_id=MY_ID, text=f"⚠️ {pair_label} PUMP +{chg5:.1f}% -> BUY paused {dur//60}min (avoid FOMO)")
                    except: pass
                    continue
                if chg5<-15:
                    s["paused"][pair_label]={"until":time.time()+dur,"reason":f"RUG {chg5:.1f}%","pct":chg5}
                    save_state(s)
                    try: await app.bot.send_message(chat_id=MY_ID, text=f"🚨 {pair_label} RUG {chg5:.1f}% -> paused {dur//3600:.1f}h")
                    except: pass
                    ...
                    if pos:
                        ...
                        if loss_pct<=-8:
                            # hard SL
                            try: await app.bot.send_message(chat_id=MY_ID, text=f"🛑 HARD SL {pair_label} {loss_pct:.1f}% - SELL @ ${closes[-1]:.4f}")
                            except: pass
        ...
        if sig=="BUY" and not pos:
            ...
            save_state(s)
            try: await app.bot.send_message(chat_id=MY_ID, text=f"🟢 BUY {pair_label} @ ${price:.4f} RSI {info['rsi']:.0f} SMA10 {info['sma10']:.2f}>{info['sma30']:.2f}")
            except: pass
            ...
            # TP loosened: was 2.5%, now 1.2% + RSI logic loosened
            elif sig=="SELL":
                sell_reason=info.get("sell_reason",f"Sell signal RSI {info['rsi']:.0f}")
                if pnl_pct>=1.2 or pnl_pct<=-3.5 or info["rsi"]>82:
                    should_sell=True
                    reason=f"{sell_reason} TP {pnl_pct:.1f}%"
                else:
                    # Don't sell tiny profit - hold longer
                    # Only sell if actually in loss or strong signal
                    if pnl_pct>=0.8 and info["rsi"]>85:
                        should_sell=True
                        reason=f"{sell_reason} TP {pnl_pct:.1f}%"
            # Trailing: if up >6%, trail 4% behind high - was 10%/5%
            high=pos.get("high",entry)
            if high>entry*1.06:
                trail_price=high*0.96
                if cur<=trail_price and pnl_pct>2:
            ...
            if should_sell:
                ...
                if s["mode"]=="DEMO":
                    ...
                    s["demo_bal"]+=proceeds
                    s["trades"].append({"type":"SELL","pair":pair_label,"price":cur,"time":datetime.now().strftime("%H:%M"),"pnl":pnl_pct,"reason":reason,"rsi":info['rsi'],"sma10":info['sma10'],"sma30":info['sma30']})
                del positions[pair_label]
                s["positions"]=positions
                save_state(s)
                try: await app.bot.send_message(chat_id=MY_ID, text=f"🔴 SELL {pair_label} @ ${cur:.4f} RSI {info['rsi']:.0f} SMA10 {info['sma10']:.2f}<{info['sma30']:.2f} {reason} PnL {pnl_pct:+.2f}%")
                except: pass
            else:
                save_state(s)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    paused_info=""
    if s.get("paused"):
        paused_info=f"\n⏸ Paused: {len(s['paused'])} pairs"
    # Check env
    env_warn=""
    if not PASSPHRASE or len(API_KEY)<10:
        env_warn="\n⚠️ API or PASSPHRASE missing! Add in justrunmyapp:\nBOT_TOKEN\nBITGET_API_KEY\nBITGET_SECRET_KEY\nBITGET_PASSPHRASE\n"
    if s["mode"]=="DEMO":
        text=f"🤖 Trader PRO v17 Hybrid\n\n🧪 DEMO MODE\nPair: {PAIRS.get(s['pair'],{}).get('emoji','')} {s['pair']}\nAlloc: ${s['alloc']}\n💰 Demo: ${s['demo_bal']:.2f}{paused_info}\nTrades: {len(s['trades'])}\nPositions: {len(s.get('positions',{}))}\nAuto: {'🟢 ON' if s['auto'] else '⚪ OFF'}\nUptime: {up//3600}h {(up%3600)//60}m{env_warn}\n\nStrategy: RSI+SMA+Adaptive Pause\nMin $1 (Custom allowed)"
    else:
        text=f"🤖 Trader PRO v17 Hybrid\n\n🔴 LIVE MODE\nPair: {PAIRS.get(s['pair'],{}).get('emoji','')} {s['pair']}\nAlloc: ${s['alloc']}\n💰 Real: ${s['real_bal']:.2f} USDT{paused_info}\nTrades: {len(s['trades'])}\nPositions: {len(s.get('positions',{}))}\nAuto: {'🟢 ON' if s['auto'] else '⚪ OFF'}\nUptime: {up//3600}h {(up%3600)//60}m{env_warn}"
    ...
    if d=="back":
        ...
        try: await q.edit_message_text(
            ...
            reply_markup=InlineKeyboardMarkup([
                ...
                [InlineKeyboardButton("✏️ Custom Amount",callback_data="custom_alloc")],
        ...
        if s["mode"]=="DEMO":
            ...
            for p,info in s.get("positions",{}).items():
                pos_txt+=f"\n📍 {p} entry ${info['price']:.4f}"
            try: await q.edit_message_text(f"🧪 DEMO Balance\n\n${s['demo_bal']:.2f} USDT{pos_txt}\nVirtual only.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅ Back",callback_data="back")]]))
            except: pass
        else:
            ...
            bal,err=await get_real_balance()
            if err:
                try: await q.edit_message_text(f"❌ {err}\n\nBitget fix: Transfer Funding->Spot", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅ Back",callback_data="back")]]))
                except: pass
            else:
        ...
        try: await q.edit_message_text("📡 Loading Bitget...")
        except: pass
        prices,src=await get_prices()
        if not prices:
            try: await q.edit_message_text(f"❌ MARKETS {src}\nCheck internet / Bitget API\n{datetime.now().strftime('%H:%M:%S')}", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅ Back",callback_data="back")]]))
            except: pass
            return
        ...
        for label in PAIRS.keys():
            ...
            if data:
                ...
                arrow="🟢" if chg>=0 else "🔴"
                paused = " ⏸" if label in s.get("paused",{}) else ""
                if p<1:
        ...
        for t in s["trades"][-8:]:
            if t['type']=="BUY":
                txt+=f"🟢 BUY {t['pair']} @ {t.get('price',0):.4f} RSI {t.get('rsi','?')}\n"
            else:
                txt+=f"🔴 SELL {t['pair']} {t.get('pnl',0):+.1f}% {t.get('reason','')[:20]}\n"
        # positions
        if s.get("positions"):
            txt+="\n📍 Open:\n"
        ...
        try: await q.edit_message_text(txt, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅ Clear All",callback_data="clear_pause")],[InlineKeyboardButton("⬅ Back",callback_data="back")]]))
        except: pass
    elif d=="clear_pause":
        s["paused"]={}; save_state(s)
        try: await q.edit_message_text("✅ Cleared paused", reply_markup=kb(s))
        except: pass
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        val=float(txt)
        if val<1:
            await update.message.reply_text("❌ Min is $1. Try again, e.g. '5'")
            return
        if val>500:
            await update.message.reply_text("❌ Max is $500. Try lower.")
            return
        s["alloc"]=val
        s["awaiting_alloc"]=False
        save_state(s)
        await update.message.reply_text(f"✅ Custom allocation set to ${val}\n\nBitget min $1, but $5+ recommended for visible profit.", reply_markup=kb(s))
    except:
        await update.message.reply_text("❌ Invalid. Just send a number like '7.5' or '12'")
        s["awaiting_alloc"]=False
        save_state(s)
...
async def main():
    ...
    print(f"v17.6 Loose TP + 24h Fix - No close cmd - API len {len(API_KEY)}")
